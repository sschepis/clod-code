...
const STATUS_META: Record<OutboundDispatchUi['status'], { icon: React.ReactNode; color: string; label: string }> = {
  ...
  rejected:         { icon: <X size={12} />, color: 'text-zinc-500', label: 'denied' },
  cancelled:        { icon: <X size={12} />, color: 'text-zinc-500', label: 'cancelled' },
};
  ...
  return (
    <div className="px-4 py-1.5 bg-zinc-950/80 border-b border-zinc-800/60 text-xs text-zinc-400 flex-shrink-0">
      ...
      >
        ...
        <span>
          <span className="text-zinc-200">{dispatches.length}</span> dispatch{dispatches.length === 1 ? '' : 'es'}
          {active > 0 && (
            ...
            return (
              <div key={d.rpcId} className="font-mono text-[11px] leading-relaxed">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-zinc-500">→ {d.peerWindowId.slice(0, 8)}</span>
                  <span className={`${meta.color}`}>{meta.label}</span>
                  <span className="text-zinc-600">sent {relTime(d.sentAt)} ago</span>
                </div>
                <div className="ml-5 text-zinc-300 truncate" title={d.task}>{d.label}</div>
                {d.error && (
                  <div className="ml-5 text-red-400 truncate" title={d.error}>error: {d.error}</div>
                )}
                {d.reason && (
                  <div className="ml-5 text-zinc-500 truncate" title={d.reason}>{d.reason}</div>
                )}
              </div>
            );
          })}
